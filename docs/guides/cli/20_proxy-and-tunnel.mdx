...
Ory Tunnel opens an HTTP tunnel to Ory, which allows to mirror Ory endpoints
on the same top-level domain as your application. This is necessary for cookies and
other security features to work properly. 

Contrary to the Ory Proxy, the incoming application traffic isn't routed through the Tunnel.
The role of the Tunnel is to allow communication with Ory, while the traffic always reaches
your application first.  

Contrary to the Proxy, Ory Tunnel does not:

  ❌ Alter incoming HTTP headers <br />
  ❌ Convert sessions into JSON Web Tokens and doesn't issue JTWs
...
:::tip
Ory Proxy and Ory Tunnel mirror Ory Cloud APIs and UIs under the `/.ory` path. 
:::

For example, when using the Ory Proxy or the Ory Tunnel, calling 
`https://<proxy-host>/.ory/ui/login` is the same as calling `https://<your-project-slug>.projects.oryapis.com/ui/login`
directly.

### Self-service Flows

Ory Proxy and Tunnel provide shortened URLs which you can use to initiate self-service flows:
